...
class ProjectCbrRaisedAPIView(APIView):
    ...
    #     sales_owner_name = UserRole.objects.get(id=int(sales_owner))
    #     project_manager = request.data.get('project_manager_name')
    ...
    #     data['sales_owner'] = str(sales_owner_name)
...
class VPRCreateView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Ensure the data is a list, wrap single dictionary input into a list
        ...
        with transaction.atomic():  # Ensures all VPRs are created together or none
            for item in data:
                serializer = VPRSerializer(data=item, context={'request': request})
                if serializer.is_valid():
                    vpr_instance = serializer.save()
                    created_instances.append(vpr_instance)
                    serializers_list.append(serializer.data)
                else:
                    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

        # Notify senior for each created VPR
        for vpr_instance in created_instances:
            self.notify_senior(vpr_instance)

        return Response(
            {"message": "VPRs created successfully", "data": serializers_list},
            status=status.HTTP_201_CREATED
        )
    ...
    def notify_senior(self, vpr):
        ...
        email = "ankit.sharma@novusinsights.com"
...
class AdvanceBillingRequisitionCreateView(APIView):
    ...
    def post(self, request):
        ...
        # created_by_role = get_user_role(created_by)
        # sales_owner = get_user_role(sales_owner)
        ...
        # data['project_manager'] = project_manager_role.id if project_manager_role else None
        # data['created_by'] = created_by_role.id if created_by_role else None
        # data['sales_owner'] = sales_owner.id if sales_owner else None
...
class VPRProjectAPIView(APIView):
    """
    API to fetch all VPR records or VPRs for a specific project if `project_id` is provided.
    - GET /vpr/all/  → Fetch all VPRs
    - GET /vpr/all/?project_id=3  → Fetch VPRs for project_id=3
    """
    authentication_classes = [JWTAuthentication]
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request,project_id=None):
        ...
        if project_id:
            queryset = VPR.objects.filter(project_id=project_id).order_by('-created_at')
        else:
            queryset = VPR.objects.all().order_by('-created_at')

        serializer = VPRSerializer(queryset, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
