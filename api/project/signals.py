...
@receiver(post_save, sender=Project)
def update_project_code(sender, instance, created, **kwargs):
    if not created:  # Ensure this only runs on updates, not on creation
        try:
            current_year_prefix = str(datetime.now().year)[-2:]
            
            # Check if the prefix already exists
            if instance.project_code[:2] == current_year_prefix:
            
                print(f"Already Updated: {instance.project_code}")
                return
            
            # Extract the remaining code after the first 3 digits
            remaining_code = instance.project_code[3:].upper() # Preserve the case and rest of code
            
            # Combine the new prefix with the remaining code
            updated_code = f"{current_year_prefix}{remaining_code}"
            
            # Prevent unnecessary saves or recursion
            if instance.project_code != updated_code:
                with transaction.atomic():
                    # Now update the code only once without recursion
                    instance.project_code = updated_code
                    
                    # very important - Only update the required fields
                    instance.save(update_fields=['project_code'])
                
                    print(f"Project Code Updated Successfully: {instance.project_code}")
        except Exception as e:
            print(f"Error updating project code: {e}")
