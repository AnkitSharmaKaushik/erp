...
class ClientViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        """
        partial = kwargs.pop('partial', False)  # Determines if PATCH or PUT
        instance = self.get_object()

        # Ensure `client` in JSON maps correctly to `client_name`
        request.data.pop('client', None)  # Remove invalid `client` field
...
class NotificationCountAPIView(APIView):
    ...
    def get(self, request):
        ...
        # Extract unique project IDs and their notification types
        project_data = list(pending_notifications.values('project', 'notification_type').distinct())

        # Convert the result into a list of dictionaries
        filtered_list = [
            {'id': item['project'], 'notification_type': item['notification_type']}
            for item in project_data if item['project'] is not None
        ]
        ...
        return Response({
            ...
            "project_id": filtered_list,  # Return list of project IDs with notification type
        }, status=200)
